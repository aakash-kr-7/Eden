# ═══════════════════════════════════════════════════════════════════
# FILE: api/chat_v4.py
# PURPOSE: Updated chat API with user typing status and mark read endpoints.
# CONTEXT: Replaces api/chat_v3.py.
# ═══════════════════════════════════════════════════════════════════
...
async def send_message(
    ...
    async with concurrency.acquire(identity.uid):
        ...
        user_analysis = await analyzer.analyze(request.message)
        user_analysis["text"] = request.message
        ...
        # 9. Return SSE stream via stream_partner_response_v2 with analysis
...
async def mark_read(
    ...
    db.mark_message_read(identity.uid, request.conversation_id, request.last_message_id)
    logger.info("Conversation %s marked read up to message %s by user %s", request.conversation_id, request.last_message_id, identity.uid)
    return {"status": "success"}

@router.post("/typing_status")
async def typing_status(
    ...
    db.update_typing_status(request.conversation_id, request.is_typing)
    logger.info("Typing status updated for conversation %s: %s", request.conversation_id, request.is_typing)
    return {"status": "success"}
